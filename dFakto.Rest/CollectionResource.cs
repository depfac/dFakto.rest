    ...
    public class CollectionResource : Resource
    {
            ...
            IEnumerable<Resource> result) : base(self)
        {
            ...
            if (request.Index > 0)
            {
                ...
                var prevUri = QueryHelpers.AddQueryString(self, queryParams);
            ...
            if (request.Index + request.Limit < total)
            {
                ...
                var nextUri = QueryHelpers.AddQueryString(self, queryParams);
                AddLink("next", nextUri);
            }

            var lastFragmentIndex = self.LastIndexOf('/');
